...
from src.constants import CREDIT_TRANSFER_TYPE, DEBIT_TRANSFER_TYPE, TRANSACTION_TYPES_CHOICES, TRANSFER_TRANSFER_TYPE
...
class CreateTransactionSerializer(serializers.Serializer):
    debit_account_number = serializers.CharField(allow_blank=True, required=False, max_length=100,
                                                 validators=[AccountValidator()])
    credit_account_number = serializers.CharField(allow_blank=True, required=False, max_length=100,
                                                  validators=[AccountValidator()])
    amount = serializers.DecimalField(max_digits=12, decimal_places=2, required=True)
    transaction_type = serializers.ChoiceField(choices=TRANSACTION_TYPES_CHOICES)

    def validate(self, attrs):
        if not attrs.get('debit_account_number') and not attrs('credit_account_number'):
            raise serializers.ValidationError('Provide either debit_account_number or credit_account_number')

        if attrs.get('transaction_type') == DEBIT_TRANSFER_TYPE and not attrs.get('debit_account_number'):
            raise serializers.ValidationError({"debit_account_number": ["Provide debit_account_number for debit"]})
        elif attrs.get('transaction_type') == CREDIT_TRANSFER_TYPE and not attrs.get('credit_account_number'):
            raise serializers.ValidationError({"credit_account_number": ["Provide credit_account_number for credit"]})
        elif attrs.get('transaction_type') == TRANSFER_TRANSFER_TYPE and (
                    not attrs.get('debit_account_number') or not attrs.get('credit_account_number')):
            raise serializers.ValidationError('Provide both debit_account_number or credit_account_number for transfer')
    ...
    def create(self, validated_data):
        ...
        if validated_data.get('transaction_type') == DEBIT_TRANSFER_TYPE:
            account_object = BankAccount.objects.get(account_number=validated_data.get('debit_account_number'))
            ...
            return update_transactions(debit_account=validated_data.get('debit_account_number'),
            ...
            account_object = BankAccount.objects.get(account_number=validated_data.get('credit_account_number'))
            account_object.balance = account_object.balance + validated_data.get('amount', 0)
            account_object.save()
            return update_transactions(credit_account=validated_data.get('credit_account_number'),
